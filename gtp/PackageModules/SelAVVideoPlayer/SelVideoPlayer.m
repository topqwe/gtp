...
- (instancetype)initWithFrame:(CGRect)frame configuration:(SelPlayerConfiguration *)configuration
{
    ...
    if (self) {
        ...
        self.playbackControls.hidden = _playerConfiguration.isHideProgress;
...
- (void)_setupPlayer
{
    if (_playerConfiguration.assetSource) {
        AVAsset* movieAsset = [AVURLAsset URLAssetWithURL:_playerConfiguration.assetSource options:nil];
    ...
    self.player.muted = _playerConfiguration.muted;
