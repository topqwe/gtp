...
@interface MoviePlayerVC () <SuperPlayerDelegate, ScanQRDelegate, UITableViewDelegate, UITableViewDataSource, CFDanmakuDelegate>
/** 播放器View的父视图*/
...
+ (instancetype)pushFromVC:(UIViewController *)rootVC requestParams:(id )requestParams success:(DataBlock)block{
    MoviePlayerVC *vc = [[MoviePlayerVC alloc] init];
    //    vc.block = block;
...
- (instancetype)init {
    if (SuperPlayerWindowShared.backController) {
        ...
        MoviePlayerVC *playerViewCtrl = (MoviePlayerVC *)SuperPlayerWindowShared.backController;
...
- (void)clickHelp:(UIButton *)sender {
    NSURL *helpUrl = [NSURL URLWithString:@"baidu.com"];
    UIApplication *myApp = [UIApplication sharedApplication];
    if ([myApp canOpenURL:helpUrl]) {
        [myApp openURL:helpUrl options:nil completionHandler:nil];
    }
}
...
- (void)viewDidLoad {
    ...
    self.videoURL = [NSString stringWithFormat:@"%@",@"http://192.74.243.238:8080/storage/slice/hls/file_202108_e874d523b25c50cff9194186ef0d9bb6/file_202108_e874d523b25c50cff9194186ef0d9bb6.m3u8"];
    if (self.videoURL) {
        SuperPlayerModel *playerModel = [[SuperPlayerModel alloc] init];
        playerModel.videoURL         = self.videoURL;
        [self.playerView playWithModel:playerModel];
        [self.playerView.controlView setTitle:[NSString stringWithFormat:@"%@",@""]];
    }
    ...
    [self.vodBtn setTitle:@"推荐列表" forState:UIControlStateNormal];
...
- (void)_refreshLiveList {
    ...
    [manager GET:@"http://xzb.qcloud.com/get_live_list2" parameters:nil headers:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if ([J2Num([responseObject valueForKeyPath:@"code"]) intValue] != 200) {
            [YKToastView showToastText:@"直播列表请求失败"];
            return;
        }
...
- (void)onNetFailed:(TXMoviePlayerNetApi *)obj reason:(NSString *)reason code:(int)code {
    [YKToastView showToastText:@"fileid请求失败"];
}
...
- (void)clickStyle:(UIButton *)btn
{
    if ([self.playerView.controlView isKindOfClass:[SPDefaultControlView class]]) {
        ...
        [YKToastView showToastText:@"已切换微博风格"];
        ...
        [YKToastView showToastText:@"已切换默认风格"];
    }
}
...
- (void)onAddClick:(UIButton *)btn
{
    ...
    self.playerView.isLockScreen = YES;
    [control addAction:[UIAlertAction actionWithTitle:@"添加视频" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        if (self.vodBtn.selected) {
            TXPlayerAuthParams *p = [TXPlayerAuthParams new];
            ...
            if (urlField.text.length == 0) {
                [YKToastView showToastText:@"请输入正确的播放地址"];
                return;
            }
