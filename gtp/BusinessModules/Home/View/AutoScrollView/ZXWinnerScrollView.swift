...
class ZXWinnerScrollView: UIView {
    ...
    fileprivate var arrList: Array<NSAttributedString> = []
    fileprivate var currentPage = 0
    fileprivate var minTotalPage = 0
    fileprivate var totalCount = 0
    
    fileprivate var arrTopList: Array<NSAttributedString> = []
    fileprivate var arrDownList: Array<NSAttributedString> = []
    ...
    ///   - width: width
    ///   - pageSize: 单页显示的条数
    @objc public convenience init(origin: CGPoint, width: CGFloat, pageSize: Int) {
        ...
        tblTopLabels.backgroundColor = .clear
        ...
        tblDownLabels.backgroundColor = .clear
    ...
    @objc public func reloadData(_ list: [NSAttributedString]) {
        self.arrList = list
    ...
    /// - Returns: -
    fileprivate func stringList(at page: Int, resetPage: ((Bool) -> Void)?) -> [NSAttributedString] {
        if totalCount > 0 {
            if totalCount > pageSize {//大于一页
                ...
                var list: Array<NSAttributedString> = []
...
extension ZXWinnerScrollView: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = ZXLotteryWinnerCell.cellWith(tableView) as! ZXLotteryWinnerCell
        
        if tableView == tblTopLabels {
            cell.configure(arrTopList[indexPath.row], indexPath)
        } else {
            cell.configure(arrDownList[indexPath.row], indexPath)
        }
        return cell
    }
