...
-(void)buildTags{
    ...
    for (int i = 0; i<_dataA.count; i++) {
        ...
        if (self.s == SearchRecordSourceTags) {
            if ([_dataA[i] isKindOfClass:[HomeItem class]]) {
                item = _dataA[i];
            }
        }else{
            word = [NSString stringWithFormat:@"%@",_dataA[i]];
        }
        
        
        
        UIButton *button =[UIButton LXButtonNoFrameWithTitle:[NSString stringWithFormat:@"%@",self.s == SearchRecordSourceTags ? item.name: word] titleFont:self.tagFont Image:nil backgroundImage:nil backgroundColor:self.s == SearchRecordSourceTags? HEXCOLOR(0xECECEC): UIColor.clearColor titleColor:self.s == SearchRecordSourceTags? [UIColor blackColor] : HEXCOLOR(0x8FAEB7)];
        
        button.layer.cornerRadius = self.s == SearchRecordSourceTags? 15 :0 ;
        button.clipsToBounds = YES;
        button.tag = self.s == SearchRecordSourceTags ?item.ID:0;
        kWeakSelf(self);
        [button addClickBlock:^(UIButton *button) {
            kStrongSelf(self);
            if (self.tagClick) {
                self.tagClick( self.s == SearchRecordSourceTags?item:word);
            }
        }];
...
-(void)layoutItems{
    ...
    CGFloat allWidth = MAINSCREEN_WIDTH;//总宽度
    ...
    for (NSUInteger i = 0; i< count; i ++) {
        ...
        NSString *tagTitle = item.titleLabel.text;
