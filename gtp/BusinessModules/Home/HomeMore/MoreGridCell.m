...
#define kGridCellHeight   140
@interface MoreGridCell()
...
+(instancetype)cellWith:(UITableView*)tabelView{
    MoreGridCell *cell = (MoreGridCell *)[tabelView dequeueReusableCellWithIdentifier:@"MoreGridCell"];
    if (!cell) {
        cell = [[MoreGridCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MoreGridCell"];
    }
    return cell;
}
...
+(CGFloat)cellHeightWithModel:(NSArray*)model{
    //VM累计出总的数据，V不在累计
    //更新GridCell高度
    return (model.count%2==0?model.count/2:model.count/2+1)*kGridCellHeight+5;
}
