...
//    _height += kDynamicsNameHeight;
    ...
    if (_model.photocollections.count != 0||_model.thumbs.count !=0) {
        [self layoutPicture];
        _height += kDynamicsNameDetailPadding;
        _height += _photoContainerSize.height;
    }
    if (_model.video.count != 0) {
        [self layoutVideo];
        _height += kDynamicsNameDetailPadding;
        _height += _videoContainerSize.height;
    }
...
{
    _detailLayout = nil;
    NSString* str = _model.dsp.length>0?_model.dsp:_model.content;
    NSMutableAttributedString * text = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@",str]];
    ...
    WS(weakSelf);
    [detector enumerateMatchesInString:[NSString stringWithFormat:@"%@",str]
                               options:kNilOptions
    ...
    YYTextContainer * container = [YYTextContainer containerWithSize:CGSizeMake(MAINSCREEN_WIDTH - 2*kDynamicsNormalPadding , _model.isOpening ? CGFLOAT_MAX : 16 * lineCount + kDynamicsLineSpacing * (lineCount - 1))];
//    YYTextContainer * container = [YYTextContainer containerWithSize:CGSizeMake(MAINSCREEN_WIDTH - kDynamicsNormalPadding - kDynamicsPortraitWidthAndHeight - kDynamicsPortraitNamePadding - kDynamicsNormalPadding, _model.isOpening ? CGFLOAT_MAX : 16 * lineCount + kDynamicsLineSpacing * (lineCount - 1))];
...
- (void)layoutVideo
{
    self.videoContainerSize = CGSizeZero;
    self.videoContainerSize = CGSizeMake( MAINSCREEN_WIDTH-2*kDynamicsNormalPadding, 210);
}
- (void)layoutPicture
{
    self.photoContainerSize = CGSizeZero;
    self.photoContainerSize = [SDWeiXinPhotoContainerView getContainerSizeWithPicPathStringsArray:_model.photocollections?_model.photocollections:_model.thumbs];
}
...
- (void)layoutThumb
{
    ...
    YYTextContainer * container = [YYTextContainer containerWithSize:CGSizeMake(MAINSCREEN_WIDTH - kDynamicsNormalPadding - kDynamicsPortraitWidthAndHeight - kDynamicsPortraitNamePadding - kDynamicsNameDetailPadding*2 - kDynamicsNormalPadding,CGFLOAT_MAX)];
