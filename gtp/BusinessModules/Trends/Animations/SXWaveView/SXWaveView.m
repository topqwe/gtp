...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        avgScoreLbl.width = W ;
        avgScoreLbl.height = W;
        avgScoreLbl.centerx = rotateImg.centerx;
        avgScoreLbl.centery = rotateImg.centery;
        avgScoreLbl.font = [UIFont systemFontOfSize:13];
        avgScoreLbl.numberOfLines = 0;
...
- (void)setPrecent:(int)precent{
    _precent = precent;
    self.avgScoreLbl.text = [NSString stringWithFormat:@"已喝\n%d杯水",precent];
...
- (void)addAnimateWithType:(int)type
{
    ...
    void(^acallBack)(CGFloat start) = ^(CGFloat start) {
        ...
        moveAction.values = [NSArray arrayWithObjects:[NSNumber numberWithFloat:-80],[NSNumber numberWithFloat:start],nil];
    ...
    if (type == 0) {
        ...
        [UIView animateWithDuration:4.0 animations:^{
            self.bigImg.top = W - ((avgScore/8.0) * W);
//            self.bigImg.top = +(self.bigImg.top-10*avgScore);
            if (avgScore == 8) {
                self.bigImg.top = -20;
            }
            if (avgScore == 1) {
