...
//        _vistor = readVisitor();
        ...
        _container.backgroundColor = RGBA(0.0, 0.0, 0.0, 0.6);
        ...
        _tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 35, ScreenWidth, ScreenHeight*3/4 - 35 - 50 - YBFrameTool.iphoneBottomHeight) style:UITableViewStyleGrouped];
...
__block NSArray* arr = @[];
[[YTSharednetManager sharedNetManager]postNetInfoWithUrl:[ApiConfig getAppApi:ApiType21] andType:All andWith:dic success:^(NSDictionary *dic) {
    if ([NSString getDataSuccessed:dic]) {
        
        HomeModel* model = [HomeModel mj_objectWithKeyValues:dic];
        arr = [HomeItem mj_objectArrayWithKeyValuesArray:model.data.list];
        
        NSDictionary* dicc = [NSString readJson2DicWithFileName:@"comments"];
        CommentListResponse *response = [[CommentListResponse alloc] initWithDictionary:dicc error:nil];
...
+(CGFloat)cellHeight:(Comment *)comment {
    ...
    return size.height + 30 + [UIFont systemFontOfSize:12.0].lineHeight * 2;
}
@end
...
- (instancetype)init {
    ...
    if(self) {
        ...
        _container = [[UIView alloc] initWithFrame:CGRectMake(0, ScreenHeight - 50 - YBFrameTool.iphoneBottomHeight, ScreenWidth, 50 + YBFrameTool.iphoneBottomHeight)];
        _container.backgroundColor = RGBA(0.0, 0.0, 0.0, 0.4);
        [self addSubview:_container];
        
        _keyboardHeight = YBFrameTool.iphoneBottomHeight;
...
- (void)updateTextViewFrame {
    CGFloat textViewHeight = _keyboardHeight > YBFrameTool.iphoneBottomHeight ? _textHeight + 2*kCommentTextViewTopBottomInset : ceilf(_textView.font.lineHeight) + 2*kCommentTextViewTopBottomInset;
