...
-(instancetype)initWithFrame:(CGRect)frame {
    ...
    if(self) {
        ...
        for(int i=0;i<_data.count;i++) {
            ...
            if(_currentIndex == i) {
                pointView.backgroundColor = YBGeneralColor.themeColor;
            }else {
        ...
        _bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, _collectionView.frame.size.height + 25, ScreenWidth, 45 + YBFrameTool.iphoneBottomHeight)];
        _bottomView.backgroundColor = [UIColor whiteColor];
        [self addSubview:_bottomView];
        
        UIView *leftView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, _itemWidth, 45 + YBFrameTool.iphoneBottomHeight)];
        leftView.backgroundColor = RGBA(230.0, 230.0, 230.0, 1.0);
...
-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    if([keyPath isEqualToString:@"attributedText"]){
        ...
        if(attributedString && attributedString.length > 0) {
            _send.backgroundColor = YBGeneralColor.themeColor;
            _send.enabled = YES;
        }else {
            _send.backgroundColor = RGBA(230.0, 230.0, 230.0, 1.0);
            _send.enabled = NO;
