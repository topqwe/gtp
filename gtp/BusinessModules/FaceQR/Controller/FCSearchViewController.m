...
- (void)addImage:(UIImage *)image{
    ...
    [faceDetector detectFaceWithReturnLandmark:NO attributes:nil completion:^(id info, NSError *error) {
        if (error) {
            hud.labelText = @"人脸检测失败,请重新添加";
            [hud hide:YES afterDelay:1.5];
        }else{
            hud.labelText = @"正在添加到人脸集合...";
        ...
        if (info) {
            ...
            if (faces.count) {
                ...
                if (faceToken && vaild) {//搜索到人脸
                    ...
                    hud.labelText = @"没有搜索到合适的人脸";
                    [hud hide:YES afterDelay:1.0];
                }
            }else{
                hud.labelText = @"没有检测到人脸";
                [hud hide:YES afterDelay:1.0];
            }
        }else{
