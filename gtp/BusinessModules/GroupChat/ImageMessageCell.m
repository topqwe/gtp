...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if(self) {
        ...
        _imageMsg.backgroundColor = [UIColor grayColor];
...
-(void)layoutSubviews {
    ...
    if([[UserInfoManager GetNSUserDefaults].data.ID isEqualToString:_chat.user_id]){
...
-(void)initData:(GroupChat *)chat {
    ...
    if(chat.picImage) {
        ...
        //chat.pic_medium.url
        [_imageMsg sd_setImageWithURL:[NSURL URLWithString:chat.content] placeholderImage:nil options:0 progress:^(NSInteger receivedSize, NSInteger expectedSize) {
...
//    [_avatar sd_setImageWithURL:[NSURL URLWithString:chat.visitor.avatar_thumbnail.url] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
//            wself.avatar.image = [image drawCircleImage];
//    }];
    if (chat.avatar.length>0) {
    NSString* title = [NSString stringWithFormat:@"mine_avator%li",[chat.avatar integerValue]];
    [_avatar setImage:[UIImage imageNamed:title]];
}
}
...
-(void)showPhotoView {
    PhotoView *photoView = [[PhotoView alloc] initWithImage:_rectImage urlPath:_chat.content];//_chat.pic_original.url
    [photoView show];
}
