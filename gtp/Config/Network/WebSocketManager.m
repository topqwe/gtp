...
-(instancetype)init {
    ...
    if(self) {
        ...
        NSURLComponents *components = [[NSURLComponents alloc] initWithURL:[NSURL URLWithString:[URL_IP stringByAppendingString:@"/groupchat"]] resolvingAgainstBaseURL:NO];
...
-(void)onNetworkStatusChange:(NSNotification *)notification {
    if(_webSocket != nil && ![YTSharednetManager isNotReachableStatus:[YTSharednetManager networkStatus]] && [self isClosed]) {
        [self reConnect];
    }
}
