...
@interface AccountPurseCell ()
@property (nonatomic, strong) NSMutableArray *btns;


@end

@implementation AccountPurseCell
...
+(instancetype)cellWith:(UITableView*)tabelView{
    AccountPurseCell *cell = (AccountPurseCell *)[tabelView dequeueReusableCellWithIdentifier:@"AccountPurseCell"];
    if (!cell) {
        cell = [[AccountPurseCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"AccountPurseCell"];
    }
    return cell;
}
...
- (void)richElementsInCellWithModel:(id)model{
    ...
    if (!dic[kTotal]||
        [dic[kTotal] isEqualToString:[NSString stringWithFormat:@"%@",@"0"]]) {
        return;
    }
    
    switch ([dic[kType]intValue]) {
        case AccountPurseTypeCredit:
            
            {
               [bt1 setTitle:[NSString stringWithFormat:@"¥ %@",dic[kTotal]] forState:UIControlStateNormal];
