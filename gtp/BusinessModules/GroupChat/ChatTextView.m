...
- (instancetype)init {
    return [self initWithFrame:MAINSCREENFRAME];
}
...
-(instancetype)initWithFrame:(CGRect)frame {
    ...
    if(self) {
        ...
        _containerBoardHeight = YBFrameTool.iphoneBottomHeight;
...
-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    if([keyPath isEqualToString:@"containerBoardHeight"]) {
        if(_containerBoardHeight == YBFrameTool.iphoneBottomHeight ){
            _container.backgroundColor = [UIColor darkGrayColor];
            _textView.textColor = [UIColor whiteColor];
...
- (void)updateContainerFrame {
    CGFloat textViewHeight = _containerBoardHeight > YBFrameTool.iphoneBottomHeight ? _textHeight + 2*kChatTextViewTopBottomInset : [UIFont systemFontOfSize:16.0].lineHeight + 2*kChatTextViewTopBottomInset;
...
- (void)updateSelectorFrame:(BOOL)animated {
    CGFloat textViewHeight = _containerBoardHeight > 0 ? _textHeight + 2*kChatTextViewTopBottomInset : [UIFont systemFontOfSize:16.0].lineHeight + 2*kChatTextViewTopBottomInset;
    if(animated) {
        ...
        switch (sender.view.tag) {
            ...
            case kChatTextViewPhotoTag: {
                ...
                if(PHAuthorizationStatusAuthorized == status) {
                    ...
                    [YKToastView showToastText:@"请在设置中开启图库读取权限"];
...
- (void)onSend {
    ...
    if(_delegate) {
        if([_textView hasText]) {
            ...
            [self hideContainerBoard];
            [YKToastView showToastText:@"请输入文字"];
        }
    }
}
...
- (void)hideContainerBoard {
    _editMessageType = EditNoneMessage;
    [self setContainerBoardHeight:YBFrameTool.iphoneBottomHeight];
    [self updateSelectorFrame:YES];
    [self updateContainerFrame];
    [_textView resignFirstResponder];
    [_emotionBtn setSelected:NO];
