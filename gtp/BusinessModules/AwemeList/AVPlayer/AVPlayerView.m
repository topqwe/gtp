...
@property (nonatomic ,strong) NSString             *movieExtension;
...
-(void)setPlayerWithUrl:(NSString *)url {
    ...
    self.movieExtension = [url containsString:@"m3u8"]?@"m3u8":@"mp4";
...
//        self.playerItem = [AVPlayerItem playerItemWithURL:self.sourceURL];
    ...
    } extension:self.movieExtension];
}
- (void)startPlayOneTime:(CGFloat)dragedSeconds{
    //转换成CMTime才能给player来控制播放进度
    CMTime dragedCMTime = CMTimeMake(dragedSeconds, 1);
    self.playCurrentTime = dragedSeconds;
...
- (void)startDownloadTask:(NSURL *)URL isBackground:(BOOL)isBackground {
    ...
    self.movieExtension = [urlStr containsString:@"m3u8"]?@"m3u8":@"mp4";
    ...
    _queryCacheOperation = [[WebCacheHelpler sharedWebCache] queryURLFromDiskMemory:_cacheFileKey cacheQueryCompletedBlock:^(id data, BOOL hasCache) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            wself.combineOperation = [[WebDownloader sharedDownloader] downloadWithURL:URL responseBlock:^(NSHTTPURLResponse *response) {
                ...
                if(!error && finished) {
                    //下载完毕，将缓存数据保存到本地
                    [[WebCacheHelpler sharedWebCache] storeDataToDiskCache:wself.data key:wself.cacheFileKey extension:self.movieExtension];
                }
            } cancelBlock:^{
...
/** 拖拽中 */
-(NSArray*)videoSliderValueChanged:(SelVideoSlider *)slider{
