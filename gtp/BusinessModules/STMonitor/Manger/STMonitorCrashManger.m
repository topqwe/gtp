...
void st_uncaughtExceptionHandler(NSException *exception) {
    ...
    NSArray * array = [NSArray arrayWithContentsOfFile:[STMonitorCrashManger defult].currentCashPath];
    ...
    BOOL isWrite =  [fecthArray writeToFile:[STMonitorCrashManger defult].currentCashPath atomically:YES];
    NSLog(@"写入状态%d",isWrite);
    
}
@interface STMonitorCrashManger()

@end
@implementation STMonitorCrashManger
+ (STMonitorCrashManger *)defult{
    static STMonitorCrashManger * defult = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        defult = [STMonitorCrashManger new];
    });
    return defult;
}
- (instancetype)init
