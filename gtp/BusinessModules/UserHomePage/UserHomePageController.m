...
#define kUserInfoHeaderHeight          350 + [YBFrameTool safeAdjustNavigationBarHeight]
...
- (void)initCollectionView {
    ...
    HoverViewFlowLayout *layout = [[HoverViewFlowLayout alloc] initWithTopHeight:[YBFrameTool safeAdjustNavigationBarHeight] + kSlideTabBarHeight];
    layout.minimumLineSpacing = 1;
    layout.minimumInteritemSpacing = 0;
    _collectionView = [[UICollectionView  alloc]initWithFrame:MAINSCREENFRAME collectionViewLayout:layout];
...
- (void)updateNavigationTitle:(CGFloat)offsetY {
    if (kUserInfoHeaderHeight - [YBFrameTool navigationBarHeight]*2 > offsetY) {
        [self.navigationController.navigationBar setTitleTextAttributes:@{NSForegroundColorAttributeName:UIColor.clearColor}];
    }
    if (kUserInfoHeaderHeight - [YBFrameTool navigationBarHeight]*2 < offsetY && offsetY < kUserInfoHeaderHeight - [YBFrameTool navigationBarHeight]) {
        CGFloat alphaRatio =  1.0f - (kUserInfoHeaderHeight - [YBFrameTool navigationBarHeight] - offsetY)/[YBFrameTool navigationBarHeight];
        [self.navigationController.navigationBar setTitleTextAttributes:@{NSForegroundColorAttributeName:[UIColor colorWithRed:1.0f green:1.0f blue:1.0f alpha:alphaRatio]}];
    }
    if (offsetY > kUserInfoHeaderHeight - [YBFrameTool navigationBarHeight]) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    AwemeLVC *controller;
    if(_tabIndex == 0) {
        controller = [[AwemeLVC alloc] initWithVideoData:_workAwemes currentIndex:indexPath.row pageIndex:_pageIndex pageSize:_pageSize awemeType:AwemeWork uid:_uid];
    }else {
        controller = [[AwemeLVC alloc] initWithVideoData:_favoriteAwemes currentIndex:indexPath.row pageIndex:_pageIndex pageSize:_pageSize awemeType:AwemeFavorite uid:_uid];
    }
...
- (void)onUserActionTap:(NSInteger)tag {
    switch (tag) {
        ...
        case UserInfoHeaderSendTag:
            [ChatListController pushFromVC:self requestParams:@1 success:^(id data) {
        ...
        case UserInfoHeaderSettingTag:{
            ...
            [menu setOnAction:^(NSInteger index) {
                [[WebCacheHelpler sharedWebCache] clearCache:^(NSString *cacheSize) {
                    [YKToastView showToastText:[NSString stringWithFormat:@"已经清除%@M缓存",cacheSize]];
                }];
            }];
            [menu show];
            break;
...
-(void)onNetworkStatusChange:(NSNotification *)notification {
    if(![YTSharednetManager isNotReachableStatus:[YTSharednetManager networkStatus]]) {
        if(_user == nil) {
            [self loadUserData];
        }
