...
-(instancetype)initWithFrame:(CGRect)frame {
    ...
    if(self) {
        self.backgroundColor = RGBA(230.0, 230.0, 230.0, 1.0);
        ...
        _bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_collectionView.frame) + 2.5, ScreenWidth, 45 + YBFrameTool.iphoneBottomHeight)];
        ...
        _album.titleLabel.font = [UIFont systemFontOfSize:16.0];
        [_album setTitle:@"相册" forState:UIControlStateNormal];
        [_album setTitleColor:YBGeneralColor.themeColor forState:UIControlStateNormal];
        ...
        _originalPhoto.hidden = YES;
        _originalPhoto.tag = kPhotoSelectorOrigPhotoTag;
        [_originalPhoto setTitleEdgeInsets:UIEdgeInsetsMake(0, 2, 0, 0)];
        _originalPhoto.titleLabel.font = [UIFont systemFontOfSize:16.0];
        [_originalPhoto setTitle:@"原图" forState:UIControlStateNormal];
        [_originalPhoto setTitleColor:YBGeneralColor.themeColor forState:UIControlStateNormal];
        ...
        _send.enabled = NO;
        _send.backgroundColor = RGBA(230.0, 230.0, 230.0, 1.0);
        _send.layer.cornerRadius = 2;
        _send.titleLabel.font = [UIFont systemFontOfSize:14.0];
...
-(void)processAssets {
    if(_selectedData.count > 9) {
        [YKToastView showToastText:@"最多选择9张图片"];
        return;
    }
    if(_delegate) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [cell setOnSelect:^(BOOL isSelected) {
        ...
        if(self.selectedData.count > 0) {
            ...
            [self.send setBackgroundColor:YBGeneralColor.themeColor];
        }else {
            self.send.enabled = NO;
            [self.send setBackgroundColor:RGBA(230.0, 230.0, 230.0, 1.0)];
        }
    }];
    return cell;
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if(self){
        ...
        _coverLayer.backgroundColor = RGBA(0.0, 0.0, 0.0, 0.6).CGColor;
        [_coverLayer setHidden:YES];
        [_photo.layer addSublayer:_coverLayer];
        
        _checkbox = [[UIButton alloc] init];
        [_checkbox setImage:[UIImage imageNamed:@"editRecordDeselected"] forState:UIControlStateNormal];
        [_checkbox setImage:[UIImage imageNamed:@"editRecordSelected"] forState:UIControlStateSelected];
